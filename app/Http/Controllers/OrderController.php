...
class OrderController extends Controller
{
    ...
    public function confirm(Request $request)
    {
        ...
        if($request->ktg_tipe !== 'joki'){
           ...
           $produk = Kategori::where('id',$item->kategori_id)->first();
            ...
            $daftarGameValidasi = ['call-of-duty', 'free-fire', 'lords-mobile', 'marvel-super-war', 'mobile-legend', 'mobile-legends','arena-of-valor','dragon-raja','hago','valorant-riot-games'];
            ...
            if(in_array($dataKategori->kode, $daftarGameValidasi)){
                ...
                } else if($dataKategori->kode == "valorant-riot-games"){
                ...
                if($data['status']['code'] == 1){
                    return response()->json([
                        'status' => false,
                        'data' => isset($data['data']['message']) ? $data['data']['message'] : 'Username tidak ditemukan atau coba lagi nanti'
                    ]);
                }
    ...
    public function store(Request $request)
    {
        ...
        if($request->payment_method == "SALDO"){
        ...
        }else if($request->payment_method == "OVO" || $request->payment_method == "GOPAY" || $request->payment_method == "SHOPEEPAY" || $request->payment_method == "DANA"){
            ...
            if($amount < 100){
                    ...
                    return response()->json(['status' => false, 'data' => 'Minimum jumlah pembayaran untuk metode pembayaran ini adalah Rp 100 Perak']);
            
            }else if($request->payment_method == "SHOPEEPAY"){
            $no_pembayaran = $api->shopeepay_admin;
            if($amount < 100){
